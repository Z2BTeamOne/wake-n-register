...
function formatFinanceOrders(_target, _orders)
{
    ...
    {(function(_idx, _arr)
        ...
        let _len = 'resource:org.acme.Z2BTestNetwork.Student#'.length;
        let _student = _arr[_idx].student.substring(_len, _arr[_idx].student.length);
        _str += '<div class="acc_header off" id="order'+_idx+'_h" target="order'+_idx+'_b"><table class="wide"><tr><th>Order #</th><th>Status</th><th class="right">Total</th><th colspan="3" class="right message">Student: '+findMember(_student,students).companyName+'</th></tr>';
...
function formatDetail(_cur, _order)
{
    ...
    _out += '<tr><td id="created">Created</td><td>'+_order.student+'</td><td>'+_order.created+'</td><td></td></tr>';
    _out += (_order.cancelled === '') ?  '<tr><td id="cancelled">'+textPrompts.financeCoOrder.cancelled+'?</td><td></td><td id="notCancel">'+textPrompts.financeCoOrder.notCancel+'</td><td></td></tr>' : '<tr><td id="cancelled">'+textPrompts.financeCoOrder.cancelled+'</td><td>'+_order.student+'</td><td>'+_order.cancelled+'</td><td></td></tr>';
    _out += (_order.bought === '') ?  '<tr><td id="purchased">'+textPrompts.financeCoOrder.purchased+'</td><td></td><td id="noPurchase">'+textPrompts.financeCoOrder.noPurchase+'</td><td></td></tr>' : '<tr><td id="purchased">'+textPrompts.financeCoOrder.purchased+'</td><td>'+_order.student+'</td><td>'+_order.bought+'</td><td></td></tr>';
    ...
    _out += (_order.disputeOpened === '') ?  '<tr><td id="disputed">'+textPrompts.financeCoOrder.disputed+'</td><td></td><td id="noDispute">'+textPrompts.financeCoOrder.noDispute+'</td><td></td></tr>' : '<tr><td id="disputed">'+textPrompts.financeCoOrder.disputed+'</td><td>'+_order.student+'</td><td>'+_order.disputeOpened+'</td><td>'+_order.dispute+'</td></tr>';
    ...
    _out += (_order.approved === '') ?  '<tr><td>Payment Approved</td><td></td><td>(No Approval from Student)</td><td></td></tr>' : '<tr><td>Payment Approved</td><td>'+_order.student+'</td><td>'+_order.approved+'</td><td></td></tr>';
